...
public class Figures 
{
    ...
    private static void christmas_tree( int height, char symbol1, char symbol2, float bulbs_proportion, int offset )
    {
        ...
        for( int i=0; i < height; i++ ) {
            ...
            for( int j=0; j < 1+2*i; j++ ) {
                System.out.printf( "%c", ( Math.random() < bulbs_proportion ? symbol2 : symbol1 ) );
            }
            System.out.println();
        }
    ...
    {
        for( int i=0; i < width; i++ ) System.out.printf( "%c", symbol );
    }
}
