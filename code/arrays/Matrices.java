...
public class Matrices
{
    ...
    private static int [] vectorMatrixProduct( int [] v, int [][] m )
    {
        if ( v.length != m.length )
            throw new RuntimeException(
                String.format( "Number of elements of the vector mismatch the number of rows of the matrix: %d != %d", v.length, m.length ) );
    ...
    private static int [] matrixVectorProduct( int [][] m, int [] v )
    {
        if ( m[0].length != v.length )
            throw new RuntimeException(
                String.format( "Number of columns of the matrix mismatch the number of elements of the vector: %d != %d", m[0].length, v.length ) );
    ...
    private static int [][] matrixMatrixProduct( int [][] a, int [][] b )
    {
        if ( a[0].length != b.length )
            throw new RuntimeException(
                String.format( "Number of columns of the first matrix mismatch the number of rows of the second one: %d != %d", a[0].length, b.length ) );
